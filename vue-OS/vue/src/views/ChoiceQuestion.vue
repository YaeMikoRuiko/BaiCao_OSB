...
<div class="neirong">
  <section class="q-container">
    <el-breadcrumb separator-class="el-icon-arrow-right" style="top: 100px;left: 100px;">
      ...
      <el-breadcrumb-item :to="{ path: '/choice' }">选择题库</el-breadcrumb-item>
      <el-breadcrumb-item><p ><strong>{{ choiceData.chapter }}[{{choiceData.id}}]</strong></p></el-breadcrumb-item>
    </el-breadcrumb>
    <div class="qinfo">
             ...
             <div class="q-body">
               <div class="q-b1" >
                 <div class="q-question" >
                   <div v-html="choiceData.question"></div>
                   <div class="q-option">
                     <div
                         class="q-item"
                         :class="{
        'correct-answer': hasAnswered && choiceData.answer === 'A',
        'wrong-answer': hasAnswered && selectedOption === 'A' && choiceData.answer !== 'A'
      }"
                         @click="checkAnswer('A')"
                         data-choice="A"
                     >
                       <div class="q-choice">A.</div>
                       {{choiceData.optionA}}
                     </div>
                     <div
                         class="q-item"
                         :class="{
        'correct-answer': hasAnswered && choiceData.answer === 'B',
        'wrong-answer': hasAnswered && selectedOption === 'B' && choiceData.answer !== 'B'
      }"
                         @click="checkAnswer('B')"
                         data-choice="B"
                     >
                       <div class="q-choice">B.</div>
                       {{choiceData.optionB}}
                     </div>
                     <div
                         class="q-item"
                         :class="{
        'correct-answer': hasAnswered && choiceData.answer === 'C',
        'wrong-answer': hasAnswered && selectedOption === 'C' && choiceData.answer !== 'C'
      }"
                         @click="checkAnswer('C')"
                         data-choice="C"
                     >
                       <div class="q-choice">C.</div>
                       {{choiceData.optionC}}
                     </div>
                     <div
                         class="q-item"
                         :class="{
        'correct-answer': hasAnswered && choiceData.answer === 'D',
        'wrong-answer': hasAnswered && selectedOption === 'D' && choiceData.answer !== 'D'
      }"
                         @click="checkAnswer('D')"
                         data-choice="D"
                     >
                       <div class="q-choice">D.</div>
                       {{choiceData.optionD}}
                     </div>
                   </div>
                 <div v-if="hasAnswered" class="answer-result"><div>正确答案:&nbsp&nbsp{{choiceData.answer}}</div>
                  解析：<div v-html="choiceData.explanation"></div>
                 </div>
               </div>
             </div>
           </div>
           ...
           <div class="q-info1" v-if="prevAndNext ">
             <div class="q-before" v-if="prevAndNext.prevChoice" style="padding: 20px" @click="getQuestion(prevAndNext.prevChoice)">
               <div class="qbf">
                 <div class="q-select" >
                   上一题
                 </div>
                 <div >
                   <a class="ub-text-muted" :href="`/choice/choiceQuestion=${this.prevAndNext.prevChoice.id}`">
                     <div class="ub-text-muted text-ellipsis" >
                       {{this.prevAndNext.prevChoice.question}}
                   </div>
                   </a>
                 </div>
               </div>
             </div>
             <div class="q-after" v-if="prevAndNext.nextChoice"  style="padding: 20px" @click="getQuestion(this.prevAndNext.nextChoice)">
               <div class="qaf">
                 <div class="q-select">
                   下一题
                 </div>
                 <div >
                   <a class="ub-text-muted" :href="`/choice/choiceQuestion=${this.prevAndNext.nextChoice.id}`">
                   <div class="ub-text-muted text-ellipsis"  >
                     {{this.prevAndNext.nextChoice.question}}
                   </div>
                   </a>
                 </div>
               </div>
...
<!--                 <el-button  style="float: right; padding: 5px 0" type="text" @click="delectComment(index)">-->
<!--                   删除-->
         ...
         <div class="qinfo-right">
            ...
            <div class="q-body">
            <div>
              <el-tag type="success" size="medium">{{choiceData.chapter}}</el-tag>
            </div>
            <div class="q-foot">
              <div class="tw-flex-grow">
                <div>
                  {{choiceData.correctCount}}
                </div>
                <div>
                  正确
                </div>
...
export default {
  ...
  methods:{
    setQuestion(){//获取题目信息
      ...
      this.choiceData=JSON.parse(a);
      this.difficulty=this.choiceData.difficulty;
    },
    checkAnswer(option) {//检查作答结果，并做出相应
      if(this.hasMethodExecuted){
        this.selectedOption = option;
        this.hasAnswered = true; // 用户点击后，设置标志为true
        let form = {
          correctCount: this.choiceData.correctCount,
          wrongCount: this.choiceData.wrongCount,
          id:this.choiceData.id
        }
        if(option===this.choiceData.answer) {
        ...
        //作答结果发送至数据库更改正确数与粗无数
          axios.put("http://localhost:8087/choice",form).then(res =>{
            console.log(res);
            if (res.data.code==='0'){
             console.log(res)
              this.setQuestion();
    ...
    setPrevAndNext(){
      axios.get("http://localhost:8087/choice/twoChoice?id="+this.choiceData.id).then(res =>{
        this.prevAndNext=res.data.data;
        console.log(this.prevAndNext)
        const div = document.createElement("div");
        if(this.prevAndNext.prevChoice){
          div.innerHTML = this.prevAndNext.prevChoice.question;
          const text = div.textContent || div.innerText;
          this.prevAndNext.prevChoice.question = "[选择题] " + text;
        }
        if(this.prevAndNext.nextChoice){
          div.innerHTML = this.prevAndNext.nextChoice.question;
          const text1 = div.textContent || div.innerText;
          this.prevAndNext.nextChoice.question = "[选择题] " + text1;
        }
    ...
    commit(){//提交评论内容
      ...
      this.userComment.questionId=this.choiceData.id;
      this.userComment.questionType="choice";
      let userCommentCopy = {
...
// 打印转换后的 JSON 数据
    ...
    setComment(){
      axios.get("http://localhost:8087/comment/choice?questionId="+this.choiceData.id)
          .then(res=> {
            console.log('成功发送数据到后台', res.data);
            // 根据后台返回的数据进行进一步处理
