...
<div class="neirong">
  <section class="q-container">
    <el-breadcrumb separator-class="el-icon-arrow-right" style="top: 100px;left: 100px;">
      ...
      <el-breadcrumb-item :to="{ path: '/short' }">简答题库</el-breadcrumb-item>
      <el-breadcrumb-item><p ><strong>{{ shortData.chapter }}[{{shortData.id}}]</strong></p></el-breadcrumb-item>
    </el-breadcrumb>
    <div class="qinfo">
         <div class="qinfo-left">
             ...
             <div class="q-body">
               <div class="q-b1" >
                 <div class="q-question" >
                   <div v-html="shortData.question"></div>
                   <div class="q-option">
                     <div class="q-item" @click="toggleAnswer(shortData)">
                         <div v-if="!shortData.showAnswer" ><i class="el-icon-zoom-in"> 点击查看答案</i></div>
                          <div v-if="shortData.showAnswer" ><i class="el-icon-zoom-out"> 点击隐藏答案</i></div>
                     </div>
                   </div>
                   <div v-if="shortData.showAnswer">
                    <i class="el-icon-s-claim">答案：</i> <div v-html="shortData.answer"></div>
                     <i class="el-icon-s-opportunity">解析：</i> <div v-html="shortData.explanation"></div>
                   </div>
                 </div>
               </div>
             </div>
           ...
           <div class="q-info1" v-if="prevAndNext ">
             <div class="q-before" v-if="prevAndNext.prevShort" style="padding: 20px" @click="getQuestion(prevAndNext.prevShort)">
               <div class="qbf">
                 <div class="q-select" >
                   上一题
                 </div>
                 <div >
                   <a class="ub-text-muted" :href="`/short/shortQuestion=${this.prevAndNext.prevShort.id}`">
                     <div class="ub-text-muted text-ellipsis" >
                       {{this.prevAndNext.prevShort.question}}
                   </div>
                   </a>
                 </div>
               </div>
             </div>
             <div class="q-after" v-if="prevAndNext.nextShort"  style="padding: 20px" @click="getQuestion(this.prevAndNext.nextShort)">
               <div class="qaf">
                 <div class="q-select">
                   下一题
                 </div>
                 <div >
                   <a class="ub-text-muted" :href="`/short/shortQuestion=${this.prevAndNext.nextShort.id}`">
                   <div class="ub-text-muted text-ellipsis" >
                     {{this.prevAndNext.nextShort.question}}
                   </div>
                   </a>
                 </div>
               </div>
         ...
         <div class="qinfo-right">
           <div class="q-panel">
            ...
            <div class="q-body">
            <div>
              <el-tag type="success" size="medium">{{shortData.chapter}}</el-tag>
            </div>
            <div class="q-foot">
              <div class="tw-flex-grow">
                <div>
                  {{shortData.correctCount}}
                </div>
                <div>
                  正确
                </div>
...
export default {
  ...
  methods:{
    setQuestion(){
      ...
      this.shortData=JSON.parse(a);
      this.difficulty=this.shortData.difficulty;
    },
    setPrevAndNext(){
      axios.get("http://localhost:8087/short/twoShort?id="+this.shortData.id).then(res =>{
        this.prevAndNext=res.data.data;
        const div = document.createElement("div");
        if(this.prevAndNext.prevShort){
          div.innerHTML = this.prevAndNext.prevShort.question;
          const text = div.textContent || div.innerText;
          this.prevAndNext.prevShort.question = "[简答题] " + text;
        }
        if(this.prevAndNext.nextShort){
          div.innerHTML = this.prevAndNext.nextShort.question;
          const text1 = div.textContent || div.innerText;
          this.prevAndNext.nextShort.question = "[简答题] " + text1;
        }
    ...
    commit(){
      ...
      this.userComment.questionId=this.shortData.id;
      this.userComment.questionType="short";
      this.userComment.account=sessionStorage.getItem("account");
      this.userComment.username=sessionStorage.getItem("username");
      let userCommentCopy = {
...
// 打印转换后的 JSON 数据
    ...
    setComment(){
      axios.get("http://localhost:8087/comment/short?questionId="+this.shortData.id)
          .then(res=> {
            console.log('成功发送数据到后台', res.data);
            // 根据后台返回的数据进行进一步处理
