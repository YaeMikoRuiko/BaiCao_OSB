...
<div class="neirong">
  <section class="q-container">
    <el-breadcrumb separator-class="el-icon-arrow-right" style="top: 100px;left: 100px;">
      ...
      <el-breadcrumb-item :to="{ path: '/fill' }">判断题库</el-breadcrumb-item>
      <el-breadcrumb-item><p ><strong>{{ fillData.chapter }}[{{fillData.id}}]</strong></p></el-breadcrumb-item>
    </el-breadcrumb>
    <div class="qinfo">
         <div class="qinfo-left">
             ...
             <div class="q-body">
               <div class="q-b1" >
                 <div class="q-question" >
                   <div v-html="fillData.question"></div>
                   <div class="q-option">
                     <div class="q-item" @click="toggleAnswer(fillData)">
                         <div v-if="!fillData.showAnswer" ><i class="el-icon-zoom-in"> 点击查看答案</i></div>
                          <div v-if="fillData.showAnswer" ><i class="el-icon-zoom-out"> 点击隐藏答案</i></div>
                     </div>
                   </div>
                   <div v-if="fillData.showAnswer">
                    <i class="el-icon-s-claim">答案：</i> <div v-html="fillData.answer"></div>
                     <i class="el-icon-s-opportunity">解析：</i> <div v-html="fillData.explanation"></div>
                   </div>
                 </div>
               </div>
             </div>
           ...
           <div class="q-info1" v-if="prevAndNext ">
             <div class="q-before" v-if="prevAndNext.prevFill" style="padding: 20px" @click="getQuestion(prevAndNext.prevFill)">
               <div class="qbf">
                 <div class="q-select" >
                   上一题
                 </div>
                 <div >
                   <a class="ub-text-muted" :href="`/fill/fillQuestion=${this.prevAndNext.prevFill.id}`">
                     <div class="ub-text-muted text-ellipsis" >
                       {{this.prevAndNext.prevFill.question}}
                   </div>
                   </a>
                 </div>
               </div>
             </div>
             <div class="q-after" v-if="prevAndNext.nextFill"  style="padding: 20px" @click="getQuestion(this.prevAndNext.nextFill)">
               <div class="qaf">
                 <div class="q-select">
                   下一题
                 </div>
                 <div >
                   <a class="ub-text-muted" :href="`/fill/fillQuestion=${this.prevAndNext.nextFill.id}`">
                   <div class="ub-text-muted text-ellipsis" >
                     {{this.prevAndNext.nextFill.question}}
                   </div>
                   </a>
                 </div>
               </div>
         ...
         <div class="qinfo-right">
           <div class="q-panel">
            ...
            <div class="q-body">
            <div>
              <el-tag type="success" size="medium">{{fillData.chapter}}</el-tag>
            </div>
            <div class="q-foot">
              <div class="tw-flex-grow">
                <div>
                  {{fillData.correctCount}}
                </div>
                <div>
                  正确
                </div>
...
export default {
  ...
  methods:{
    setQuestion(){
      ...
      this.fillData=JSON.parse(a);
      this.difficulty=this.fillData.difficulty;
    },
    setPrevAndNext(){
      axios.get("http://localhost:8087/fill/twoFill?id="+this.fillData.id).then(res =>{
        this.prevAndNext=res.data.data;
        const div = document.createElement("div");
        if(this.prevAndNext.prevFill){
          div.innerHTML = this.prevAndNext.prevFill.question;
          const text = div.textContent || div.innerText;
          this.prevAndNext.prevFill.question = "[填空题] " + text;
        }
        if(this.prevAndNext.nextFill){
          div.innerHTML = this.prevAndNext.nextFill.question;
          const text1 = div.textContent || div.innerText;
          this.prevAndNext.nextFill.question = "[填空题] " + text1;
        }
    ...
    commit(){
      ...
      this.userComment.questionId=this.fillData.id;
      this.userComment.questionType="fill";
      this.userComment.account=sessionStorage.getItem("account");
      this.userComment.username=sessionStorage.getItem("username");
      let userCommentCopy = {
...
// 打印转换后的 JSON 数据
    ...
    setComment(){
      axios.get("http://localhost:8087/comment/fill?questionId="+this.fillData.id)
          .then(res=> {
            console.log('成功发送数据到后台', res.data);
            // 根据后台返回的数据进行进一步处理
