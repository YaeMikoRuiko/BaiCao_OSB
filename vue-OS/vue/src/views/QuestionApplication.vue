...
<!--              <el-input v-model="form.question"></el-input>-->
              ...
              <el-form-item label="正确答案">
                <myEditor editorId="editor3" :initialValue="form.answer" @editorDataChanged="handleEditorDataChanged" />
              </el-form-item>
...
export default {
  ...
  methods:{
    load(){
      axios.get("http://localhost:8087/application",{
        params: {
    ...
    save(){
      if (this.form.id){//更新
        axios.put("http://localhost:8087/application",this.form).then(res =>{
          console.log(res);
          if (res.data.code==='0'){
            this.$message({
              type: "success",
        ...
        axios.post("http://localhost:8087/application",this.form).then(res =>{
          console.log(res);
          if (res.data.code==='0'){
            this.$message({
              type: "success",
    ...
    handleDelete(id){
      console.log(id);
      axios.delete("http://localhost:8087/application/"+id).then(res => {
        if (res.data.code==='0'){
          this.$message({
            type: "success",
            message: "删除成功"
